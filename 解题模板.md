...
```python
# for横向遍历   backtrack纵向遍历
def backtracking(xxx,xxx,startIndex,path,result): 
  	"""
  	startIndex
  	path
  	result
    ...
    if 终止条件:
        存放结果
        return
    for i in range(startIndex,len(xxx)):
        处理节点
        backtrack(xxx,xxx,total,i,path,result) # i
        回溯，撤销处理结果
# 定义全局变量
result=[]
path=[]
startIndex=0 # 控制递归过程中遍历的起点，从而避免产生重复的组合
backtracking(xxx,xxx,startIndex,path,result)
```
