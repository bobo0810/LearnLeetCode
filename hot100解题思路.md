...
题目：未排序的整数数组 nums ，找出数字连续的最长序列

（不要求序列元素在原数组中连续）      

思路：每个数判断是否为连续序列的开头。

1. 跳过非开头：用set判断是否包含当前元素-1。 
2. 判断递增：持续判断是否在集合里
...
- `prefix[右]- 区间和k = prefix[左]`，则`prefix[左]`次数满足条件
...
```python
 def subarray_sum(nums, k):
    ...
    prefix_sum = 0
    count = 0
    prefix_map = {0:1}  # 初始前缀和为0的出现次数为1
    
    # 遍历数组
    for num in nums:
        prefix_sum += num  # 更新当前前缀和
        
        # 如果存在 prefix_sum - k，则说明有子数组和为 k
        if prefix_sum - k in prefix_sum_map.keys():
...
思路：三路快排，即三个指针，划分为左右区间，左区间有序，右区间无序，因为尚未遍历。
...
def sortColors(nums):
    ...
    while mid <= high: # 终止条件 mid往右  high往左，两者相遇
...
def mergeKLists(lists):
    ...
    while heap:
        ...
        current.next = node  # 加入结果链表
        current = current.next
        if node.next:  # 如果还有后续节点，加入堆
...
def getIntersectionNode(headA, headB):
      ...
      while cur_A!=cur_B: # 相同节点 或都为None
          if cur_A:
              cur_A=cur_A.next
          else:
              cur_A=headB
...
def largestRectangleArea(heights):
    ...
    for i in range(len(heights)):
        ...
        if len(stack) == 0: # 栈为空，当前元素直接入栈
            ...
            stack.append(i)
        elif heights[i] < stack[-1]:  # 当前元素 < 栈顶元素，持续出栈弹栈并计算面积
...
|                | 思路                   | 参数                                                         | 终止条件                                      |
| -------------- | ---------------------- | ------------------------------------------------------------ | --------------------------------------------- |
| 是否能到达     | 持续更新==最远下标==   | cur_index当前最远下标                                        | （1）已越界（2）能到达                        |
| 到达的最少步数 | ==最少步数==走最远范围 | cur_index 当前最远下标<br />next_index 多跳一次的最远下标<br />step 记录结果 | （1）next_index已越界（2）cur_index当前能到达 |
