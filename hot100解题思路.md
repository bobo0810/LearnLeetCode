...
#### ⭐️1. 完全平方数：和为 n 的完全平方数的最少数量 。例如13 = 2²+3²
...
1. dp[i]定义  和为i的最少数量
2. 公式  `dp[j]=min(dp[j-i²], dp[j])`    i取值范围`1~√n`
3. 初始化  `dp[0]=0`  为求最小值，故dp初始化为无穷大

```python
dp = [float('inf')] * (n + 1)
dp[0] = 0
for i in range(1, int(n ** 0.5) + 1):  # 先物品 1~根号n
    ...
    def longestPalindrome(self, s: str) -> str:
        ...
        def expand(s,left,right):
            ...
            while left>=0 and right<len(s) and s[left]==s[right]:
                left-=1
                right+=1
            return s[left+1:right] # while终止时回文子串区间left+1 ~ right-1 左闭右开
...
1. dp定义: dp[i]表示**以第i个元素结尾**的最长递增子序列的长度。 i取值0~n-1，对应元素nums[i]
