...
- `prefix[右]- 区间和k = prefix[左]`，则`prefix[左]`的次数即为所求

```python
 def subarray_sum(nums, k):
    # 前缀和 和 哈希表
    pre_sum = 0
    pre_map = {0:1}  # 初始化，前缀和为0的出现次数为1
    # 记录结果
    count = 0
    
    for num in nums:
        pre_sum += num  # 更新当前前缀和
        # 如果存在pre_sum - k，则说明有子数组和为k
...
思路：三路快排，即三个指针，划分为左右区间，左区间有序 ==left mid==，右区间无序==right==，因为尚未遍历。
...
硬币每次==独立事件==，互不影响。故答案是1/2

思路：条件概率公式
$$
P(A \mid B) = \frac{P(A \cap B)}{P(B)}
$$
A第二次为正面; B第一次为正面。因为独立，所以如下
$$
