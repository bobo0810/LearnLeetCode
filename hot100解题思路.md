...
2. 面积= height  * （右边第一个小的下标值  -  左边第一个小于的下标值 -1 ）
...
1. cur_index   当前最远到达的下标
2. next_index   在cur_index范围内，每走一步都更新 下次最远到达的下标
3. step  记录最小步数
...
def jump(self, nums: List[int]) -> int:
    ...
    step=0
    for i in range(len(nums)):
        next_index= max(next_index,i+nums[i]) # 每步都更新下次最远到达的下标
        if i>cur_index:
            return -1 # 无法到达
        elif i>cur_index:
            continue  # 继续走
