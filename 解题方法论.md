...
| 组合 | startIndex                         | len(path)==len(nums)                      | range(startIndex,len(nums))                                  | backtrack(nums,i+1) <br />i重复取   i+1不重复 |
| 分割 | startIndex切割线                   | startIndex==len(s)                        | range(startIndex, len(s)) <br />切出的子串`s[startIndex:i+1]` | backtrack(s,i+1)                              |
...
def backtracking(nums,startIndex):
    ...
    for i in range(startIndex,len(nums)): # 横向遍历的起始位置
        path.append(nums[i])
        backtrack(nums,i)  #递归 i依赖于横向，控制纵向。i表示可重复,i+1不能重复取(画出N叉树就明白了)
        path.pop()
startIndex=0 
backtracking(nums,startIndex)
return result 
...
1. 背包容量从==0==开始
2. 明确3个问题   01背包or完全背包、组合or排列、具体问题

![image-20250101下午62832654](./assets/image-20250101下午62832654.png)
