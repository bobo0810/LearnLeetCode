...
3. 遍历过程 （**明确**for横向遍历什么   递归纵向遍历什么    path存的什么）
...
- 切割问题递归如何终止？ 切完整个字符串终止
- 在递归循环中如何截取子串？s[startIndex: i+1]代表下标startIndex~i
- 如何判断回文？ s==s[::-1]

```python
# 131.分割回文串: 分割成子串，每个子串都是回文串
result = [] # 全局参数存结果
def backtracking(path,startIndex,s):
    # 终止条件
    if startIndex == len(s):
        result.append(path.copy()) # 必须复制
        return
    for i in range(startIndex, len(s)):
        if s[startIndex: i+1] == s[startIndex: i + 1][::-1]: # 若反序和正序相同，则是回文串 
            path.append(s[startIndex:i+1])
            self.backtracking(s, i+1, path, result) # 纵向 从下一字符切割，判断其余是否仍为回文串
            path.pop() # 回溯
path=[]
startIndex=0 # 切割线
backtracking(path,startIndex,s)
return result
```
