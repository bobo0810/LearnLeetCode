...
|            | mid比较对象 | 思路                                                 |
| ---------- | ----------- | ---------------------------------------------------- |
| 查找目标值 | left        | 先判断左右区间有序，再判断目标值位于哪个区间         |
| 查找最小值 | right       | 旋转点一定在无序区间内，用变量min_value持续更新mid值 |
...
2. 遍历字符串 `right<len(s)`   right是即将遍历的下标
   ...
   收缩两步骤  （a）记录结果（b）左边界元素是否更新窗口参数（valid、windows、left）
...
1. 背包容量从==0==开始
2. 明确3个问题   01背包or完全背包（确定正序or逆序）、组合or排列（确定顺序）、具体子问题
