...
def partition(nums, left, right):
   ...
   i,j=left,right
   while i<j:
      while i < j and nums[j] >= nums[left]: # 右指针找到第一个小于基准的元素
         j -= 1
      while i < j and nums[i] <= nums[left]: # 左指针找到第一个大于基准的元素
         i += 1
      nums[i], nums[j] = nums[j], nums[i] # 交换两者
    nums[left], nums[i] = nums[i], nums[left] # 基准元素放到对应位置，返回最终位置
    return i
```
