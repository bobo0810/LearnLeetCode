...
def left_bound(nums: List[int], target: int) -> int:
    ...
    while left <= right:
        ...
        if nums[mid] < target:
            ...
            result = mid # 记录结果，不断更新结果
            right = mid - 1 # 寻找左边界，故收缩右边界
    return result
```
...
[核心思想](https://www.hello-algo.com/chapter_heap/heap/#3)  搞懂「建堆」、「调整」和「删除」的过程。
...
def findKthLargest(nums, k):
    ...
    def max_heap(nums,length,i):
        ...
        if left>=length and right>=length:  # 下标0~n-1   n已越界
            return 
    ...
    for i in range(len(nums)-1,len(nums)-k,-1): # 左闭右开，差值就是元素个数
        ...
        # i指堆范围，持续缩小堆范围 从0~n-1缩小为0~n-k+1,代表移除k-1个元素
        # 0 代表 每轮都完整从根节点开始堆化
        max_heap(nums,i,0) 
    return nums[0]
# 示例
...
1. 参数
   ...
   参数：path存放结果，result存放结果集

   求组合用startIndex，求排列用used

2. 终止条件 、记录结果一定要复制

3. 遍历过程 （**明确**for横向遍历什么和取值范围   递归纵向遍历什么和取值范围    path存的什么）
...
def backtracking(nums,startIndex):
    ...
    for i in range(startIndex, len(nums)): # 横向遍历的起始位置
        path.append(nums[i])
        self.backtracking(nums, i + 1, path, result) # #递归 i控制纵向遍历位置。i+1不能重复取
        path.pop()
startIndex=0
...
```python
result=[] # 全局参数，记录结果
def backtracking(path,startIndex,xxx): 
    if 终止条件:
        result.append(path.copy())  # 存放结果 !必须复制!
        return   #返回
    for i in range(startIndex,len(xxx)): # 横向遍历的起始位置
        处理当前值
        backtrack(path,i,xxx)  #递归 i控制纵向遍历位置，依赖于横向。i表示可重复,i+1不能重复取(画出N叉树就明白了)
        撤销当前值  
path=[]
startIndex=0 # 横向遍历的起始位置
backtracking(path,startIndex,xxx)
return result 
```
...
path=[]
used=[False] * len(nums) # 求排列有序 跳过已使用过的元素
backtrack(path,nums,used)
return result
```
