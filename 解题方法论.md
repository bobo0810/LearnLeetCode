...
def minWindow(self, s: str, t: str) -> str:
    ...
    # 窗口参数
    window = {} # 无重复字符用Set()   统计字符频率用Counter
   	valid=0 # 单字符满足频次时+1   值等于need长度时满足条件
  	left，right, = 0,0 # 左闭右开[left,right)   区间[0, 0)中没有元素，避免边界问题  
    ...
    while right<len(s):  # right是待处理的索引，取值范围是0~n-1    此时窗口内范围是left~right-1
        ...
        if char in need.keys():
            ...
            if window[char] == need[char]:
                 valid += 1 # 满足一个字符的出现频次后加1

        # 3. 判断左侧窗口是否收缩
        # while right-left >= len(t): # 固定窗口
        while left<right and valid==len(need): # 动态窗口
            # 记录结果
