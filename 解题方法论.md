...
1. 确定传参
   ...
   - 局部参数：求排列用used，其余用used

2. 确定终止条件    保存结果**必须复制**
   - 子集：`startIndex == len(nums)` 遍历完
   - 组合： startIndex  `len(path)==len(nums)`  
   - 切割：`startIndex == len(nums)` 遍历完
   - 排列：`used`     满足 `len(path)==len(nums)`  
