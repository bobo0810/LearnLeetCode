...
4. 顺序： 
   ...
   ```python
   for i in range(1, n): #物品
       for j in range(bagweight + 1): # 背包 0~n
           if j < weight[i]: #背包容量 无法塞入物品i
               dp[i][j] = dp[i-1][j]
           else:
...
3. 初始化
   ...
   ```python
   dp = [0] * (bagweight + 1)  # 初始值为0，背包容量从0开始填充，故+1
...
```python
for i in range(n): # 物品
    for j in range(bagweight, weight[i]-1, -1):  # 背包倒序  取值范围背包容量~物品i重量
...
for i in range(len(weight)): 
    for j in range(weight[i], bagweight + 1):  # 取值范围 物品i重量~背包容量
        dp[j] = max(dp[j], dp[j - weight[i]] + value[i])
```
...
即有几种方法（有顺序，如爬楼梯）：**唯一特例 先背包后物品**    

（背包容量的每一个值，都是经过 物品a 和 b 的计算，包含了{a, b} 和 {b, a}两种情况）
