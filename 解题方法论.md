...
def merge_two_lists(l1, l2): # 归并排序
    ...
    while l1 and l2:
        if l1.val < l2.val:
            cur_node.next = l1
            l1 = l1.next  # 更新l1表头
            cur_node = cur_node.next # 更新新链表表头
        elif l1.val > l2.val:
            current.next = l2
            l2 = l2.next  # 更新l2表头
            cur_node = cur_node.next # 更新新链表表头
...
|                  | dp定义                                                     | 递推公式                                                     |
| ---------------- | ---------------------------------------------------------- | ------------------------------------------------------------ |
| 最短编辑距离     | `dp[i][j]` word1前i个字符   word2前j个字符的最小编辑距离   |                                                              |
| 最长公共子序列   | `dp[i][j]`  text1前i个字符，text2前j个字符的最长公共子序列 | 当前字符相同，则`dp[i][j] = dp[i-1][j-1]+1` <br />当前字符不同，则`dp[i][j] = max(dp[i-1][j], dp[i][j-1])` |
| 最长连续递增序列 | `dp[i]` 以==第i个元素结尾==的最长递增子序列的长度          |                                                              |
| 最长回文子串     | 用双指针解决                                               |                                                              |
