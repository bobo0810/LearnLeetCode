...
- 搜索目标值：根据左右区间的有序性和目标值范围调整指针
  ...
  1. 判断哪个区间有序或已达边界   
  2. 判断是否在有序区间内，否则在无序区间。无法判断则left+1继续
     1. nums[left] <  nums[mid]     
     2. nums[left] >   nums[mid]  
     3. nums[left] ==   nums[mid]  到达左边界，无法判断。left+=1

- 寻找最小值：

  思路：只需比较中点mid与==右端点right==

  1. mid >right, 最小值在右区间
  2. mid<=right  最小值在左区间或中点
